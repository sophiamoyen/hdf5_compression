...
out_folder = "./interactive_recordings_small_4/"
downsampling = 15
...
def copy_dataset(name, demo, compressed):
    ...
    if name not in compression_list:
        ...
        image = d
        dataset = compressed.create_dataset(name, data=image, maxshape=(image.shape[0], image.shape[1], image.shape[2]),
                    compression="gzip", compression_opts = 0, dtype=np.uint8, chunks=True)
